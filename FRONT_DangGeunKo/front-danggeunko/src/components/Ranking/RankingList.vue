    ...
    <div class="course-panel" id="coursePanel">
        <div class="course-grid" v-if="rankingCourseList && rankingCourseList.length > 0">
            ...
            <div v-for="(course, index) in rankingCourseList" :key="course.courseId"
                ...
                <div class="ranking-card" @click="goDetail(course)">
...
onMounted(async () => {
    // 만약 watch에 immediate가 없다면 여기서만 한 번 호출
    if (rankingCourseList.value.length === 0) {
        await courseStore.getWeeklyRanking();
    }
});
...
.course-item-wrapper.has-ai-comment .ranking-card {
    flex-basis: 50%;
    flex-shrink: 0;
    margin-right: 0;
